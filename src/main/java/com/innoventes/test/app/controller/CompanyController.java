...
public class CompanyController {
	...
	//Task 3 - adding API for id
	@GetMapping("/getCompanyById/{id}")
	public Company getCompanyById(@Valid @PathVariable String id) {
		Long idLong = Long.parseLong(id);
		//URI location = ServletUriComponentsBuilder.fromCurrentRequest().build().toUri();
		//return ResponseEntity.status(HttpStatus.OK).location(location).body(companyService.getCompanyById(id));
		return companyService.getCompanyById(idLong);
	}
	
	//Task 4 - adding API for companyCode
	@GetMapping("/getCompanyByCompanyCode/{companyCode}")
	public Company getCompanyByCompanyCode(@PathVariable String companyCode) {
		//URI location = ServletUriComponentsBuilder.fromCurrentRequest().build().toUri();
		//return ResponseEntity.status(HttpStatus.OK).location(location).body(companyService.getCompanyByCompanyCode(companyCode));
